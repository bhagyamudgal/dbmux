...
export class PostgresDriver implements DatabaseDriver {
    ...
    async getTableInfo(
        ...
        const schemaIdentifier = this.formatAsIdentifier(schema);
        const tableIdentifier = this.formatAsIdentifier(tableName);
        const countResult = await this.pool.query(
            `SELECT COUNT(*) as count FROM ${schemaIdentifier}.${tableIdentifier}`
        );
